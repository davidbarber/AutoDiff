...
Ntrain=length(seq)
    ...
    H=2 # size of the latent representation
    X=2 # size of the word vec representation
    ...
    (totalloss,c)=mapreduce!(Fx,Fxpy,includedloss[2:Tmax],node)
    ...
    sc=1:2 # just do this for sequence 1 to see if it compiles ok:
    for tind=1:Tmax-1
        println(tind)
        value[x[tind]]=zeros(1,length(sc))
    end
    ...
    for t=Tmax-length(seq[1])+1:Tmax-1
        counter+=1
        value[x[t]][s]=seq[1][counter]
    end
    for t=2:Tmax
        value[lossIDX[t]][s]=0 # initialise all to zero
    end

    for t=2:Tmax
        value[target[t]][s]=BitArray(W,1)
    end

    counter=0
    for t=Tmax-length(seq[s])+2:Tmax
        counter+=1
        fill!(value[target[t][s]],false)
        value[target[t]][seq[s][counter+1]]=true
        value[lossIDX[t]]=1
    end
    ...
    #    t=1
       ...
       for sc=minibatch
            ...
            for tind=Tmax-length(seq[sc])+2:Tmax
                ...
                value[target[tind]][seq[sc][counter+1]]=1
