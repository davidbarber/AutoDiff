...
FXtransposePY_inplace(handle,value,auxvalue,x,y)=Fxpy_inplace(value,auxvalue,x',y)

function DXtransposePY(handle,derivativeIDX,f_c,faux_c,grad_c,grad_n,x::Array,y::Array)
    if derivativeIDX==1
        if size(x)==(1,1)
...
if PROC=="GPU"
    function FXtransposePY_inplace(handle,value::CudaArray,auxvalue,x::CudaArray,y::CudaArray)
        if size(x)==(1,1)
    ...
    function DXtransposePY(handle,derivativeIDX,f_c,faux_c,grad_c,grad_n,x::CudaArray,y::CudaArray)
        if derivativeIDX==1
            if size(x)==(1,1)
                axpy!(1.0,sum(grad_c),grad_n)
            else
