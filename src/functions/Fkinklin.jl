...
Dkinklin(handle,derivativeIDX,f_c,faux_c,grad_c,grad_n,x)=axpy!(1.0,grad_c.*(gamma+(1-gamma)*(x.>0)),grad_n)

if PROC=="GPU"
    Fkinklin_inplace(handle,value,aux,x::CudaArray)=kinklin!(x,value)
